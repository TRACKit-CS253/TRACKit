...
export default function Lectures({ role }) {
  ...
  const [existingFiles, setExistingFiles] = useState([]);  // New state for existing files
  const [filesToDelete, setFilesToDelete] = useState([]);  // New state to track files marked for deletion
  const [showFileDeleteConfirm, setShowFileDeleteConfirm] = useState(false); // New state for file deletion confirmation
  const [fileToDelete, setFileToDelete] = useState(null); // New state to track which file is being deleted
  ...
  const [showHeadingDeleteConfirm, setShowHeadingDeleteConfirm] = useState(false);
  ...
  const handleEditClick = async (e, lecture) => {
    e.stopPropagation();
    ...
    if (lecture.fileUrls && lecture.fileUrls.length > 0) {
      setExistingFiles(lecture.fileUrls);
    } else {
      setExistingFiles([]);
    }
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      // If we're editing and have files to delete, send that information
      if (formType === 'edit' && filesToDelete.length > 0) {
  ...
  const handleDeleteHeadingClick = (e, heading) => {
    e.stopPropagation();
    setHeadingToDelete(heading);
    setShowHeadingDeleteConfirm(true);
  };

  const handleDeleteHeadingConfirm = async () => {
    if (!headingToDelete) return;

    try {
      const token = localStorage.getItem('token');
      const response = await axios.delete(
        `${process.env.REACT_APP_API_URL}/api/lectures/${courseDetails.id}/heading`,
        {
          headers: { Authorization: `Bearer ${token}` },
          data: {
            heading: headingToDelete
          }
        }
      );
      ...
      if (response.data.success) {
        ...
        showNotification(response.data.message || 'Failed to delete module', 'error');
      }
    } catch (err) {
      console.error('Error deleting module:', err);
      showNotification('Error deleting module', 'error');
    } finally {
      setShowHeadingDeleteConfirm(false);
      setHeadingToDelete(null);
    }
  };

  // Handle removing a new file from the upload list
  const handleRemoveFile = (indexToRemove) => {
    setPdfFiles(pdfFiles.filter((_, index) => index !== indexToRemove));
  };

  // Update the handle remove existing file function to show confirmation
  const handleRemoveExistingFile = (file) => {
    setFileToDelete(file);
    setShowFileDeleteConfirm(true);
  };

  // Add new function to handle confirmed file deletion
  const handleFileDeleteConfirm = () => {
    if (!fileToDelete) return;
    
    // Add the file URL to the files to delete list
    setFilesToDelete([...filesToDelete, fileToDelete.url]);
    
    // Remove the file from the display list of existing files
    setExistingFiles(existingFiles.filter(file => file.url !== fileToDelete.url));
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-blue-50">
                  ...
                  <div className="flex items-center gap-3">
                    {role !== "student" && (
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          onClick={(e) => handleDeleteHeadingClick(e, heading)}
      ...
      {(showHeadingForm || showForm || showDeleteConfirm || showSubsectionForm || 
        ...
        showSubheadingDeleteConfirm || showHeadingDeleteConfirm || showFileDeleteConfirm) && (
        ...
        <div className="fixed inset-0 flex items-center justify-center z-[60] p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-3xl w-full p-6 animate-modal-in overflow-y-auto max-h-[90vh] border border-gray-100">
            ...
            <form onSubmit={handleFormSubmit} className="space-y-6">
              ...
              {formType === 'edit' && existingFiles.length > 0 && (
                <div>
                  ...
                  <div className="p-4 bg-gray-50 rounded-xl border border-gray-200">
                    <ul className="max-h-36 overflow-y-auto space-y-1">
                      {existingFiles.map((file, index) => (
                        <li key={`existing-${index}`} className="text-sm text-gray-700 flex items-center gap-2 py-1.5 px-2 rounded-lg hover:bg-blue-50 transition-colors">
              ...
              <div>
                <label className="block mb-2 text-sm font-medium text-gray-700">
                  {formType === 'create' ? 'Upload Supplementary Files' : 'Upload Additional Files'}
                </label>
                <div className="border-2 border-dashed border-gray-200 rounded-xl p-8 bg-gray-50 bg-opacity-70 hover:bg-opacity-100 transition-all">
                  ...
                  {/* New file list with remove buttons */}
                  {pdfFiles.length > 0 && (
        ...
        <div className="fixed inset-0 flex items-center justify-center z-[60] p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6 animate-modal-in border border-gray-100">
            <div className="flex items-start gap-4 mb-4">
              ...
              <div>
                <h2 className="text-xl font-bold text-gray-800">Confirm Module Deletion</h2>
                <p className="text-gray-600 mt-1">Are you sure you want to delete the module "{headingToDelete}"?</p>
              </div>
            </div>
            ...
            <div className="bg-red-50 border border-red-100 rounded-xl p-4 mb-6">
              <p className="text-red-600 font-medium flex items-start gap-2">
                <span className="mt-0.5">⚠️</span>
                <span>Warning: This will delete all sections and lectures in this module. This action cannot be undone.</span>
              </p>
            </div>
            <div className="flex justify-end gap-3">
              <button
                type="button"
                onClick={() => setShowHeadingDeleteConfirm(false)}
                className="px-5 py-2.5 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-all"
              >
                Cancel
              </button>
              <button
                type="button"
                onClick={handleDeleteHeadingConfirm}
                className="px-5 py-2.5 bg-red-600 hover:bg-red-700 text-white rounded-lg shadow-md hover:shadow-lg transition-all"
              >
                Delete Module
              </button>
      ...
      {/* Add the File Delete Confirmation Modal */}
        ...
        <div className="fixed inset-0 flex items-center justify-center z-[60] p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6 animate-modal-in border border-gray-100">
            <div className="flex items-start gap-4 mb-6">
              ...
              <div>
                <h2 className="text-xl font-bold text-gray-800">Confirm File Removal</h2>
                <p className="text-gray-600 mt-1">
                  Are you sure you want to remove this file: <span className="font-medium">{fileToDelete?.name}</span>?
                </p>
                <div className="mt-3 bg-yellow-50 p-3 rounded-lg border border-yellow-100">
                  <p className="text-yellow-700 text-sm flex items-center gap-2">
                    <span>⚠️</span>
                    <span>This file will be permanently deleted when you save the lecture.</span>
                  </p>
                </div>
              </div>
            </div>
