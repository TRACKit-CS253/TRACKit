...
export default function AddFaculty() {
  ...
  const handleManualSubmit = async (e) => {
    ...
    if (!/^[A-Za-z\s]+$/.test(facultyData.firstName)) {
      setError('First name should contain only alphabets and spaces');
      return;
    }
    
    if (facultyData.lastName && !/^[A-Za-z\s]+$/.test(facultyData.lastName)) {
  ...
  const handleBulkSubmit = async (e) => {
    ...
    try {
      ...
      const errorMessage = error.response?.data?.message || 'An unexpected error occurred while uploading faculty';
                    ...
                    <div className="relative">
                      ...
                      <div className="relative">
                        <select
                          ...
                          onChange={handleChange}
                          required
                          className="w-full appearance-none px-4 py-2.5 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white text-gray-700 pr-10 transition-all duration-200"
                        >
